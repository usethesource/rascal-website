...
* On the Unix commandline the `${EDITOR}` environment variable will be used to open a file. If that file is present in the local file system, it is opened by running `${EDITOR} /absolute/path/to/module.rsc`, but if the file is hidden behind an opaque [Location](../../../Rascal/Expressions/Values/Location/index.md) scheme, then it is first copied to a temporary file on the local file system, and then opened.
