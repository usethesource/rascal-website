...
Note that [Pattern Matching][Rascal concepts:Pattern matching](../../../../..//RascalConcepts/PatternMatching) will _ignore_ all trees in layout positions, such that the parse tree of "1 + \\n1" will match against `<Exp e1> + <Exp e2>`. The same goes for equality on parse trees.
