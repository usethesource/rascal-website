...
If `methodASTs` would have been an empty set, then the [search pattern][Pattern matching](/docs//RascalConcepts/PatternMatching) `/Declaration d` or the condition `d.decl == ...` would have failed on this example. But it didn't! It found exactly one match.
