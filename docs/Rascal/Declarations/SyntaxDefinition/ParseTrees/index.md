...
*  Most internal nodes are applications (`appl`) of a `Production` to a list of children `Tree` nodes. `Production` is the abstract representation of a [syntax definition](../../../../Rascal/Declarations/SyntaxDefinition/index.md) rule, which consists of a definition of an alternative for a `Symbol` by a list of `Symbols`.
