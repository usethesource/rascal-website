...
The `rascal-shell`, `rascal-prepare` and `rascal-commands` blocks offer the option to take screenshots of interative content.

Every time the shell detects web ((Library:Content-Content)) as a top-level REPL result:
...
* the image is included in the documentation _at the very spot_ of the command that generated it.
* the code block is either finished or continues with the next command.
