...
One of the topics of RAP 17 (Error Tree Support) was how handle regular code that tries to access parts of error trees that are not present ("beyond the dot"). This RAP
builds on this by also including access to tree parts where the tree happens to be an abmiguity node.

## Motivation

Currently when code tries to access part of a tree but the tree is an ambiguous at the top level, the interpreter throws a static error as if the error was
...
Because static errors cannot be caught, there is now way for a developer to handle this situation without explicitly checking beforehand
