...
* The goal is _not_ to invent a generic safe concurrent programming model, the goal is _only_ to allow Rascal code to run concurrently without stepping out of the language.  
...
* Run-time semantics of fork:  
  ...
  * The return values of the closures are collected in an array at their spawning-index, every closure _must_ return a value.  
...
* The cancellation policy on exceptions is the hardest part to implement, it may interact with the _monitor feature_ of Rascal, which also has a cancellation feature.  
...
* No additional features to the static checker seem necessary, however:  
  ...
  * The goal would be to compile closures defined _inside_ the fork loop without changing the compiler for the closure functionality.
