...
* Bridging to JVM languages:  
  ...
  * \`IValueFactory.number({String, float,double,int,long,byte\[\] twoscomplement} num,)\` converts the value *without loss of the original precision*.  
    * Java Axiom \`X x \= … ; vf.number(x).as\<X\>() \== x\`, for X in {int, double, float, T  
...
* We arrive at axioms which were not possible due to number coercion and operator overloading:  
  * \`(x \+ y) \+ z \== x \+ (y \+ z)\`  
  * \`x \== y \<=\> “\<x\>” \== \<y\>”\`, the “WYSIWYG” axiom, but care has to be taken that a canonical printing operation exists (see above) and that it never accidentally rounds numbers.  
* Since there is only one kind of number left, we can use all arithmetic operators to combine any kind of number.   
...
* Ugly notation of repetents with double brackets. \`a.b((c))\`  
  * Hard to fix  
